...
pub async fn run_bench(
    ...
    let tokens_per_forward_pass_list =
        results.iter().filter_map(|result| result.tokens_per_forward_pass).collect::<Vec<f64>>();
    ...
    let tokens_per_forward_pass_metric = calculate_metric(&tokens_per_forward_pass_list);
    ...
    table
        ...
        .add_row(vec!["Accepted per pass, tok", tokens_per_forward_pass_metric.as_str()]);
