...
/// # Safety
///
/// `base[offset .. offset + k]` must be in bounds of the source buffer.
#[inline(always)]
...
/// # Safety
///
/// `row * k + k` must be within the activation buffer, and for `Int8` the
/// scales buffer must hold `row * ceil(k / group_size) + ceil(k / group_size)`
/// entries; the dispatcher derives both from the validated matmul shape.
...
/// # Safety
///
/// With `transpose`, `b_col * leading_dimension + a_row.len()` must be within
/// the weight buffer; without it, `(a_row.len() - 1) * leading_dimension + b_col`
/// must be. `b_col` can originate from gather indices (data, not a loop
/// counter), so the callers assert it against the row count.
#[inline(always)]
...
/// # Safety
///
/// Same bounds contract as [`dot_full_precision_typed`], with `weights`
/// reinterpreted at `weights_data_type` width.
#[inline]
...
/// `b_col * k * bits / 8`, scales/biases at `b_col * num_groups_k`, and
/// zero-points at `b_col * zero_point_stride` must all be in bounds.
/// `b_col` can come from gather indices (data, not a loop counter), so the
/// caller asserts it against the buffer-derived row bound first.
#[inline]
...
impl MatmulKernel for MatmulCpuKernel {
    ...
    fn encode<'a, 'b, 'd, TB: BufferArg<'b, Cpu>>(
        ...
        // Gather indices are data, not loop counters: derive a bound on
        // addressable B rows from the weight buffer extent so the compute
        // closure can debug-assert every gathered index before dereferencing.
        let b_row_bound = if gather_ptr.is_none() {
            n_u
        } else {
            match &b {
                MatmulB::FullPrecision {
                    ...
                    let (_, _, byte_len) = b.into_parts();
                    let leading_dimension = b_leading_dimension
                        .map(|ld| ld as usize)
                        .unwrap_or(if b_transpose { k_u } else { n_u });
                    let row_bytes = leading_dimension * weights_data_type.size_in_bytes();
                    if b_transpose && row_bytes > 0 {
                        byte_len / row_bytes
                    } else {
                        usize::MAX
                    }
                ...
                | MatmulB::ScaleSymmetricDequant {
                    ...
                    let byte_len = b.as_buffer_range_ref().range().len();
                    let row_bits = k_u * DataType::from(*mode).size_in_bits();
                    (byte_len * 8).checked_div(row_bits).unwrap_or(usize::MAX)
                },
            }
        };
