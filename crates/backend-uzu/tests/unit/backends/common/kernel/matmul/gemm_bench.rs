...
use crate::{
    ...
    tests::{
        ...
        matmul::{bench_fp_gemm_shapes, gemma3_batched_layer_shapes, iter_encode_loop},
...
// Full-precision GEMV at draft-verification batch sizes (same forms as the
// quant group): on bandwidth-bound bf16 forms the batched tile should bring
// the m > 1 pass close to the m = 1 pass. Run with UZU_GEMV_M_TILE=4|8.
#[uzu_bench]
fn bench_fp_gemv_batched(c: &mut Criterion) {
    let context = crate::tests::util::shared_metal_context();
    let mut kernel = <<Metal as Backend>::Kernels as Kernels>::MatmulKernel::new(
        &context,
        bf16::data_type(),
    ...
    let mut group = c.benchmark_group(format!("{}/Kernel/FpGemvBatched/Gemma3_BF16", type_short_name::<Metal>()));
    for (layer, shape) in gemma3_batched_layer_shapes() {
        let (m, k, n) = (shape.m, shape.k, shape.n);
        let a = alloc_allocation::<Metal, bf16>(&context, m as usize * k as usize);
        let b_weights = alloc_allocation::<Metal, bf16>(&context, n as usize * k as usize);
        let mut d = alloc_allocation::<Metal, bf16>(&context, m as usize * n as usize);
        group.throughput(Throughput::Elements(u64::from(m) * u64::from(k) * u64::from(n)));
        group.bench_function(BenchmarkId::from_parameter(format!("{layer}_{shape}")), |bencher| {
