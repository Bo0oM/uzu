...
fn run_matmul_opts(
    ...
    let a_allocation = alloc_allocation_with_data::<Cpu, f32>(&context, a);
    let b_allocation = alloc_allocation_with_data::<Cpu, f32>(&context, b);
    let gather_allocation = gather.map(|indices| alloc_allocation_with_data::<Cpu, u32>(&context, indices));
    let mut d_allocation = alloc_allocation::<Cpu, f32>(&context, (m * n) as usize);
    ...
    kernel
        .encode(
            MatmulArguments {
                ...
                d: &mut d_allocation,
                d_transform: MatmulDOps::none(),
                gather_indices: gather_allocation.as_ref(),
                m,
                n,
                k,
            },
...
// The untransposed branch reads B with a per-element leading-dimension
// stride and was rewritten to consume the predecoded activation row; it
// needs its own reference and thread-identity coverage.
#[uzu_test]
fn threaded_matmul_untransposed() {
    for (m, n, k) in [(1u32, 300u32, 128u32), (8, 129, 64), (3, 37, 19)] {
        let a = random_matrix((m * k) as usize, 0xA0761D6478BD642F ^ u64::from(n));
        let b = random_matrix((k * n) as usize, 0xE7037ED1A0B428DB ^ u64::from(k));

        let single = run_matmul_opts(1, m, n, k, &a, &b, false, None);
        for row in 0..m as usize {
            for col in 0..n as usize {
                let mut expected = 0.0f32;
                let mut magnitude = 0.0f32;
                for inner in 0..k as usize {
                    let term = a[row * k as usize + inner] * b[inner * n as usize + col];
                    expected += term;
                    magnitude += term.abs();
                }
                let value = single[row * n as usize + col];
                let tolerance = 2.0 * k as f32 * f32::EPSILON * magnitude;
                assert!(
                    (value - expected).abs() <= tolerance,
                    "untransposed {m}x{n}x{k} [{row},{col}]: {value} vs {expected}"
                );
            }
        }
        for threads in [2usize, 5, 16] {
            assert_eq!(run_matmul_opts(threads, m, n, k, &a, &b, false, None), single, "threads={threads}");
        }
    }
}

// Gather remaps output column `col` to B-row `gather_indices[row * n + col]`
// (the embedding-readout lookup path); indices are data, so the rewritten
// column-parallel kernel must agree with the reference for arbitrary rows.
#[uzu_test]
fn threaded_matmul_gather() {
    let (m, n, k, b_rows) = (3u32, 41u32, 64u32, 97u32);
    let a = random_matrix((m * k) as usize, 0x8EBC6AF09C88C6E3);
    let b = random_matrix((b_rows * k) as usize, 0x589965CC75374CC3);
    let mut rng = SmallRng::seed_from_u64(0x1D8E4E27C47D124F);
    let gather: Vec<u32> = (0..(m * n) as usize).map(|_| rng.random_range(0..b_rows)).collect();

    let single = run_matmul_opts(1, m, n, k, &a, &b, true, Some(&gather));
    for row in 0..m as usize {
        for col in 0..n as usize {
            let b_row = gather[row * n as usize + col] as usize;
            let mut expected = 0.0f32;
            let mut magnitude = 0.0f32;
            for inner in 0..k as usize {
                let term = a[row * k as usize + inner] * b[b_row * k as usize + inner];
                expected += term;
                magnitude += term.abs();
            }
            let value = single[row * n as usize + col];
            ...
            assert!(
                (value - expected).abs() <= tolerance,
                "gather {m}x{n}x{k} [{row},{col}] b_row={b_row}: {value} vs {expected}"
            );
        }
    }
    for threads in 2..=17usize {
        assert_eq!(run_matmul_opts(threads, m, n, k, &a, &b, true, Some(&gather)), single, "threads={threads}");
    }
}
